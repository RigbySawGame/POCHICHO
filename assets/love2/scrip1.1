...
const centralGlow = createGlowMaterial('rgba(255,200,230,0.9)', 180, 0.38);
centralGlow.scale.set(11, 11, 1);
scene.add(centralGlow);

// Segundo halo dorado, más grande y más sutil
const goldenGlow = createGlowMaterial('rgba(255,210,130,0.7)', 200, 0.22);
goldenGlow.scale.set(18, 18, 1);
scene.add(goldenGlow);

// --- NUBES DE NEBULOSA ---
// Crea 15 manchas de colores aleatorios en el fondo del espacio
// Cambiar "15" para más o menos nebulosas
// Paleta romántica para nebulosas: rosas, violetas, dorados
const nebulaHues = [320, 300, 340, 280, 30, 50, 310, 290, 350, 20, 270, 330, 45, 315, 285];
for (let i = 0; i < 15; i++) {
    const hue = nebulaHues[i % nebulaHues.length];
    const sat = 70 + Math.random() * 20; // 70-90% saturación
    const color = `hsla(${hue}, ${sat}%, 60%, 0.55)`;
...
const galaxyParameters = {
    ...
    insideColor: new THREE.Color(0xff85c2), // Rosa suave interior

    // Color de las partículas en los bordes exteriores
    outsideColor: new THREE.Color(0xffd580), // Dorado suave exterior
};
...
for (let i = 0; i < galaxyParameters.count; i++) {
    ...
    const mixedColor = new THREE.Color(0xffaadd); // Rosa pastel interior
    mixedColor.lerp(new THREE.Color(0xffe0a0), radius / galaxyParameters.radius); // Dorado suave exterior
...
for (let group = 0; group < numGroups; group++) {
    ...
    for (let i = 0; i < pointsPerGroup; i++) {
        ...
        // Color de lejos: rosa-dorado romántico (se mezcla con galaxia)
        const colorFar = new THREE.Color(0xffaacc); // Rosa suave
        colorFar.lerp(new THREE.Color(0xffd480), radius / galaxyParameters.radius); // → dorado
        colorFar.multiplyScalar(0.8 + 0.2 * Math.random());
...
function createShootingStar() {
    ...
    const trailLength = 160; // Estela más larga para efecto romántico
    ...
    const atmosphereMaterial = new THREE.ShaderMaterial({
        uniforms: {
            glowColor: { value: new THREE.Color(0xffb3e6) } // Halo rosa suave romántico
    ...
    const trailMaterial = new THREE.LineBasicMaterial({
        color: 0xffccee,      // Color de la estela: rosa suave romántico
        transparent: true,
        opacity: 0.85,
        linewidth: 2
    });
    ...
    shootingStarGroup.userData = {
        ...
        speed: 0.0007 + Math.random() * 0.0008, // más lentas y suaves
        life: 0,
        maxLife: 420,            // Más largas en pantalla
...
function createPlanetTexture(size = 512) {
    ...
    gradient.addColorStop(0.00, '#fff0f8'); // Centro: blanco rosado suave
    gradient.addColorStop(0.12, '#ffb3d9'); // Rosa suave
    gradient.addColorStop(0.25, '#ff80bf'); // Rosa medio
    gradient.addColorStop(0.38, '#e6b3ff'); // Lavanda pálida
    gradient.addColorStop(0.52, '#cc66ff'); // Violeta
    gradient.addColorStop(0.65, '#ffcc66'); // Dorado cálido
    gradient.addColorStop(0.80, '#ff9966'); // Durazno/cobre
    gradient.addColorStop(1.00, '#c266ff'); // Morado suave
    ctx.fillStyle = gradient;
    ctx.fillRect(0, 0, size, size);

    // Manchas de color aleatorias (simulan nubes/tormentas)
    const spotColors = ['#fff0f8','#ffb3d9','#ff80bf','#e6b3ff','#cc66ff','#ffcc66','#ff9966','#ffd9f0','#f0c0ff'];
...
const stormShader = {
    ...
    fragmentShader: `
        ...
        void main() {
            ...
            texColor.rgb += noise * vec3(0.9, 0.5, 0.7); // Tono rosado-dorado suave
...
function createTextRings() {
    ...
    for (let i = 0; i < numRings; i++) {
        ...
        ctx.fillStyle   = '#ffe0a0'; // Dorado suave romántico
                                     // → Cambiar para texto de otro color
        ctx.textAlign   = 'left';
        ctx.textBaseline = 'alphabetic';

        // Capa 1: Borde exterior del texto (glow)
        ctx.shadowColor  = '#ffb347'; // Halo dorado cálido
        ctx.shadowBlur   = 28;
        ctx.lineWidth    = 6;
        ctx.strokeStyle  = '#fff8e7'; // Borde blanco cálido
        ctx.strokeText(fullText, 0, textureHeight * 0.8);

        // Capa 2: Relleno con efecto glow dorado-rosado
        ctx.shadowColor  = '#ffccaa'; // Halo dorado rosado
        ctx.shadowBlur   = 20;
...
function createPoemGalaxy() {
    ...
    allPoems[0].forEach((line, idx) => {
        ...
        cx.font         = `italic ${idx === 0 ? 46 : 33}px "Cormorant Garamond", serif`; // ← tamaño fuente
        cx.textAlign    = 'center';
        cx.textBaseline = 'middle';
        cx.shadowColor  = idx === 0 ? 'rgba(255,180,230,1.0)' : 'rgba(255,220,150,0.95)';
        cx.shadowBlur   = 28;
        cx.fillStyle    = idx === 0 ? '#ffe0f5' : '#fff5cc'; // Rosa pálido / dorado pálido
...
function updatePoemGalaxy(time) {
    ...
    if (poemOpacity <= 0) {
        ...
        poem.forEach((line, idx) => {
            ...
            const cx = c.getContext('2d');
            cx.clearRect(0, 0, cW, cH);
            cx.font         = `italic ${idx === 0 ? 46 : 33}px "Cormorant Garamond", serif`;
            cx.textAlign    = 'center';
            cx.textBaseline = 'middle';
            cx.shadowColor  = idx === 0 ? 'rgba(255,180,230,1.0)' : 'rgba(255,220,150,0.95)';
            cx.shadowBlur   = 28;
...
function createFallingHearts() {
    ...
    function spawnHeart() {
        ...
        // Dibuja el emoji corazón (variedad romántica)
        const heartEmojis = ['💖','💕','💗','💓','💝','✨','💫'];
       ...
       // Posición sobre el BORDE del corazón
        ...
        sprite.scale.set(5, 5, 1); // corazones más grandes

        sprite.userData.speedY    = -(0.015 + Math.random() * 0.035); // caída más suave
        sprite.userData.speedX    = (Math.random() - 0.5) * 0.015;
        sprite.userData.opacity   = 1.0;
        sprite.userData.baseY     = sprite.position.y;
        sprite.userData.life      = 0;
        sprite.userData.maxLife   = 160 + Math.floor(Math.random() * 100);
    ...
    setInterval(() => {
        ...
        if (window.fallingHearts.length < 60) { // más corazones simultáneos
            spawnHeart();
        }
    }, 150); // aparecen más seguido
}
